...
def get_df(data_size:int=100, path:str="Data\AAPL_stock_prices.csv", delimeter: str = ',', from_end: bool = True, date_column: str = 'Date', target_column: str = 'Close') -> tuple[np.ndarray, MinMaxScaler, MinMaxScaler, pd.DataFrame]:
    ...
    scaled_target_data = target_scaler.fit_transform(df[target_column].values.reshape(-1, 1))
    scaled_data = scaler.fit_transform(df.drop(columns=[target_column]).values)
    merged_data = np.hstack((scaled_target_data, scaled_data))
    print(type(merged_data))
    return merged_data, target_scaler, scaler, df
...
def create_dataset(data: np.ndarray, time_step: int=10, output_window_size: int = 1) -> tuple[np.ndarray, np.ndarray]:
    ...
    for i in range(len(data) - time_step):
        ...
        seq_x, seq_y = data[i:input_end_index, :], data[input_end_index:output_end_index, 0].reshape(-1, 1)
...
def predict(X_train, X_test, model: Sequential):
...
data, target_scaler, scaler, *_ = get_df(data_size=500)
X: np.ndarray
Y: np.ndarray
